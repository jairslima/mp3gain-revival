...
TMP_BASE="$REPO_ROOT/test/_tmp_smoke"
mkdir -p "$TMP_BASE"
TMP_DIR="$(mktemp -d "$TMP_BASE/run.XXXXXX")"
...
restore
out=$("$EXE" "$(to_cli_path "$T1")" "$(to_cli_path "$T2")" 2>&1)
...
restore
"$EXE" -r "$(to_cli_path "$T1")" > /dev/null 2>&1
out=$("$EXE" "$(to_cli_path "$T1")" 2>&1)
...
restore
"$EXE" -r "$(to_cli_path "$T1")" > /dev/null 2>&1
"$EXE" -u "$(to_cli_path "$T1")" > /dev/null 2>&1
out=$("$EXE" "$(to_cli_path "$T1")" 2>&1)
...
restore
"$EXE" -a "$(to_cli_path "$T1")" "$(to_cli_path "$T2")" > /dev/null 2>&1
out1=$("$EXE" "$(to_cli_path "$T1")" 2>&1)
out2=$("$EXE" "$(to_cli_path "$T2")" 2>&1)
...
echo "[7] Exit codes"
restore
"$EXE" "$(to_cli_path "$T1")" > /dev/null 2>&1
code=$?
...
restore
set +e
out=$("$EXE" "$(to_cli_path "$BAD")" "$(to_cli_path "$T1")" 2>&1)
code=$?
set -e
if [ "$code" -eq 1 ]; then pass "mixed batch exits 1"; else fail "mixed batch exits 1 (got $code)"; fi
check "corrupted fixture reports missing frames" "Can't find any valid MP3 frames in file $(to_cli_path "$BAD")" "$out"
...
restore
"$EXE" -r "$(to_cli_path "$T1")" > /dev/null 2>&1
out=$("$EXE" -s c "$(to_cli_path "$T1")" 2>&1)
...
"$EXE" -s d "$(to_cli_path "$T1")" > /dev/null 2>&1
out=$("$EXE" -s c "$(to_cli_path "$T1")" 2>&1)
...
restore
"$EXE" -s i -r "$(to_cli_path "$T1")" > /dev/null 2>&1
out_id3=$("$EXE" -s i -s c "$(to_cli_path "$T1")" 2>&1)
out_ape=$("$EXE" -s a -s c "$(to_cli_path "$T1")" 2>&1)
...
restore
out=$("$EXE" -k -d 20 -r "$(to_cli_path "$T1")" 2>&1)
